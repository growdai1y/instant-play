...
flowchart TB
    ...
    MANAGE["즉시 재생 플랫폼(100)<br/>업체·콘텐츠 등록 관리부(110)"]
    VERIFY_REQUEST["즉시 재생 플랫폼(100)<br/>등록 재생 검증부(170)<br/>검증용 권한 요청"]
    PTEST["외부 VOD 업체 시스템(200)<br/>검증용 권한·시험 재생정보"]
    VERIFY_RUN["즉시 재생 플랫폼(100)<br/>등록 재생 검증부(170)<br/>후보 구성별 실제 재생 시험"]
    REG_STORE[("저장소(190)<br/>등록 정보·버전")]
    VERIFIED_STORE[("저장소(190)<br/>검증된 재생구성집합")]
    RECREG["즉시 재생 플랫폼(100)<br/>추천 콘텐츠 등록부(130)"]
...
```mermaid
flowchart TB
    SOURCE["조건 수행 결과 보고<br/>조건 실행·원천 결과 보고부(330)<br/>광고 완료·결제 승인·프로모션 적용"]

    subgraph PLATFORM_REQUEST["플랫폼 요청 단계(100)"]
        direction TB
        CONDITION["조건 판정·결과 데이터<br/>처리부(140)"]
        REQUEST["권한 요청 생성·전송<br/>권한 요청·응답 검증부(160)"]
        TX_INITIAL[("권한 거래 최초 기록<br/>거래·처리상태 연계부(180)")]
    ...
    subgraph PROVIDER["VOD 업체 판단 단계(200)"]
        direction TB
        RESULTCHECK["조건 결과의 형식·규칙·버전 확인<br/>조건 결과 확인부(210)"]
        RIGHTSCHECK["업체 고유 권한정책 판단<br/>업체 권한정책 판정부(220)"]
        RECORD{"수락·거절·처리상태<br/>기록부(230)"}
        ISSUE["한시적 권한·실행 정보 발급<br/>재생권한 발급부(240)"]
    ...
    subgraph PLATFORM_RESPONSE["플랫폼 응답 단계(100)"]
        direction TB
        RESPONSE["업체 응답 수신·검증<br/>권한 요청·응답 검증부(160)"]
        TX_UPDATE[("같은 권한 거래 상태 갱신<br/>거래·처리상태 연계부(180)")]
...
```mermaid
flowchart TB
    SOURCE330["1. 조건 수행 결과 수집·보고<br/>조건 실행·원천 결과 보고부(330)<br/>광고 완료·결제 승인·프로모션 적용"]
    DECISION140["2. 콘텐츠별 조건 충족 판정<br/>조건 판정·결과 데이터 처리부(140)"]
    PROVIDER200["3. 업체가 요청을 수락한 경우<br/>한시적 권한·재생 방식·<br/>동적 재생정보 응답<br/>외부 VOD 업체 시스템(200)"]
    CONFIG200["4. 업체가 이번에 보낸<br/>재생 방식과 버전"]
    VERIFIED170[("사전 시험 합격 목록<br/>등록 재생 검증부(170)")]
    CHECK_VERIFIED{"5. 업체가 보낸 방식이<br/>사전 시험 합격 목록에 있는가?<br/>권한 요청·응답 검증부(160)"}
    CAPABILITY340["사용자 기기가 지원하는<br/>재생 방식<br/>즉시 콘텐츠 재생기(340)"]
    CHECK_DEVICE{"6. 사용자 기기에서<br/>실행 가능한가?<br/>권한 요청·응답 검증부(160)"}
...
flowchart TB
    subgraph P["즉시 재생 플랫폼(100)"]
        direction TB
        R["업체·콘텐츠<br/>등록 관리부(110)"]
        V["등록 재생<br/>검증부(170)"]
        S[("등록 콘텐츠 정보<br/>조건·결과 프로파일<br/>검증된 재생구성집합")]
        RR["추천 콘텐츠<br/>등록부(130)"]
        Q["선택 콘텐츠의<br/>사전 조건 조회"]
        C["조건 판정·<br/>결과 데이터 처리부(140)"]
        G["권한 요청·<br/>응답 검증부(160)"]
    ...
    subgraph D["사용자 단말(300)"]
        ...
        D1["선택 콘텐츠의 사전 조건 실행<br/>원천 결과 보고부(330)"]
