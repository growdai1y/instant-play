...
1. **발명의 대상 및 핵심 구조**
   ...
   - 업체 등록 시 플랫폼은 VOD 업체가 지원하는 스트리밍 방식, DRM 체계, 코덱, 권한 요청규격 및 라이선스 제공방식을 정적 재생 연동 프로필로 등록한다.
   ...
   - VOD 업체가 반환한 선택 재생방식, 단기 재생 URL, 접근 토큰, 매니페스트 위치 또는 DRM 라이선스 연계정보를 즉시 재생기에 제공하여 콘텐츠를 재생한다.
...
5. **출원 전 확정하거나 치환할 정보**
   ...
   - DRM 라이선스 정보의 단말 전달방법과 플랫폼 서버·단말 DRM 모듈 사이의 처리경계
...
| 실제 재생정보 | 사전 저장된 URL 또는 업체 앱 내부정보 사용 | 단말 재생능력에 부합하도록 업체가 선택한 방식과 단기 URL·토큰·매니페스트·DRM 라이선스 정보를 권한 응답으로 지연 취득 |
...
flowchart TB
    ...
    subgraph NEW["본 발명 아이디어 적용 흐름"]
        ...
        N5 --> N6["업체 선택 방식+단기 URL·토큰·DRM 정보 취득"]
...
- **정적 재생 연동 프로필:** VOD 업체가 지원하는 스트리밍 방식, DRM 체계, 코덱, 권한 요청규격 및 라이선스 제공방식처럼 업체 연동과 호환성 판단에 사용하는 정보이다. 실제 콘텐츠 URL·접근 토큰·DRM 키와 구별된다.
- **단말 재생능력 정보:** 사용자 단말의 즉시 재생기가 지원하는 스트리밍 방식, DRM 체계, 비디오·오디오 코덱 및 선택적인 해상도·보안수준을 나타내는 정보이다.
- **실제 재생정보:** 콘텐츠를 실제 요청·재생하기 위해 필요한 단기 URL, 매니페스트 위치, 접근 토큰, DRM 라이선스 취득정보 또는 이들의 조합이다.
...
첫째, 복수 VOD 업체의 콘텐츠를 하나의 플랫폼에서 추천하려면 플랫폼이 콘텐츠 정보를 알아야 하지만, 실제 재생 URL·DRM 정보까지 사전에 저장하면 보안위험, 주소 만료, 업체 정책변경 및 데이터 동기화 문제가 발생한다.
...
또 다른 목적은 업체가 발급한 단기 URL·접근 토큰·매니페스트 위치·DRM 연계정보를 사용자 재생 시점에 지연 취득하여, 등록정보의 노후화와 실제 재생정보의 장기 노출을 줄이는 것이다.
...
또 다른 목적은 업체의 정적 재생 연동 프로필과 개별 사용자 단말의 재생능력 정보를 대조하여 호환되는 스트리밍·DRM·코덱 조합을 재생 시점에 선택하고, DRM 콘텐츠 키를 플랫폼 서버에 노출하지 않으면서 기존 단말 DRM 모듈을 이용하여 보호 콘텐츠를 재생하는 것이다.

##### 3) 본 발명의 해결수단 요약

본 발명은 즉시 재생 플랫폼, VOD 업체 시스템, 사용자 단말 및 추천 시스템을 포함한다.

1. VOD 업체는 플랫폼이 제공하는 업체 등록 인터페이스를 통해 업체정보, 지원 스트리밍 방식, DRM 체계, 코덱, 권한 요청규격 및 라이선스 제공방식을 포함하는 정적 재생 연동 프로필을 등록한다.
2. VOD 업체는 콘텐츠 등록 인터페이스를 통해 업체 콘텐츠 식별자, 추천용 콘텐츠 메타데이터 및 상기 정적 재생 연동 프로필을 지시하는 참조값을 등록한다.
3. 플랫폼은 업체 콘텐츠 식별자와 추천용 메타데이터 및 재생 연동 프로필 참조값을 동일 플랫폼 콘텐츠 식별자에 연결하여 저장하되, 실제 콘텐츠 URL·영구 접근 토큰·DRM 키는 등록정보로 저장하지 않는다.
...
9. 플랫폼은 선택 재생방식, 단기 URL·접근 토큰·매니페스트 및 DRM 라이선스 취득정보를 즉시 재생기에 제공한다. DRM이 적용된 경우 즉시 재생기는 업체 연계 라이선스 시스템과 기존 단말 DRM 모듈을 이용하여 단말 결합 라이선스를 취득한 후 콘텐츠를 재생한다.
...
```mermaid
flowchart LR
    VOD["VOD 업체 시스템(200)<br/>정적 재생 연동 프로필·콘텐츠 등록<br/>호환방식 선택·권한·재생정보 발급"]
    ...
    VOD <-->|"업체·재생 프로필·콘텐츠 등록 / 결과"| MANAGE
    ...
    GRANT -->|"선택 방식+단기 URL·토큰·라이선스 정보"| CTRL
    PLAYER <-->|"DRM 라이선스 직접 취득"| DRM
```

즉시 재생 콘텐츠 저장소는 업체 단위의 정적 재생 연동 프로필과 이를 참조하는 업체 콘텐츠 식별자·추천용 메타데이터를 저장하지만, 실제 콘텐츠 URL이나 장기 접근 토큰은 저장하지 않는다. 콘텐츠 재생권한 획득부는 광고 완료 후 저장된 업체 식별자와 단말 재생능력을 사용하여 VOD 업체 시스템에 권한을 요청하고, 업체가 선택한 호환 재생방식과 실제 재생정보를 취득한다. DRM이 적용되면 사용자 단말의 즉시 콘텐츠 재생기는 업체 연계 DRM 라이선스 시스템과 직접 통신하고 기존 단말 DRM 모듈로 라이선스를 처리할 수 있다.
...
7. **VOD 업체 시스템(200)**: 업체·재생 연동 프로필·콘텐츠 등록정보를 제공한다. 플랫폼이 전송한 업체 콘텐츠 식별자와 단말 재생능력을 확인하여 호환되는 실제 재생방식을 선택하고, 한시적 권한과 실제 재생정보를 발급하며 콘텐츠 및 연계 DRM 라이선스를 제공한다.
...
11. **즉시 콘텐츠 재생기 또는 멀티미디어 재생기(340)**: 단말이 지원하는 스트리밍·DRM·코덱 능력정보를 즉시 재생 제어부에 제공한다. 광고와 VOD 콘텐츠를 재생하고, DRM이 적용된 경우 업체가 반환한 라이선스 취득정보를 이용하여 기존 단말 DRM 모듈을 통해 라이선스를 취득하며 재생결과를 반환한다.
...
4. **실제 재생정보**: VOD 콘텐츠의 재생을 시작하기 위해 필요한 단기 URL, 매니페스트 위치, 접근 토큰, DRM 라이선스 취득정보 또는 이들의 조합이다.
...
9. **정적 재생 연동 프로필**: 업체 단위로 등록되는 지원 스트리밍 방식, DRM 체계, 비디오·오디오 코덱, 권한 요청규격, 인증방식 및 라이선스 제공방식의 집합이다. 복수 프로필을 등록하고 개별 콘텐츠가 하나 이상을 참조할 수 있다.
...
12. **DRM 라이선스 취득정보**: 즉시 콘텐츠 재생기가 업체 연계 DRM 라이선스 시스템에 라이선스를 요청하는 데 사용하는 라이선스 주소, 단기 라이선스 토큰, DRM 체계 식별자 또는 이들의 조합이다. 콘텐츠 키 자체와 구별된다.

##### 4) 업체 재생 연동 프로필 및 콘텐츠 등록 레코드 JSON

업체 재생 연동 프로필은 VOD 업체가 지원하는 재생방식과 권한·라이선스 연동규격을 업체 단위로 등록한 정적 정보이다. 개별 콘텐츠 등록 레코드는 이 프로필의 식별자를 참조한다. 이에 따라 동일 업체의 다수 콘텐츠에 실제 스트리밍 주소를 반복 저장하지 않고도 지원방식을 확인할 수 있다.

`providerContentId`와 `playbackResourceId`는 업체가 제공하는 불투명 식별자이고, `recommendationMetadata`는 추천을 위해 실제 값으로 저장된다. `grantEndpoint`는 업체 권한 서비스를 호출하기 위한 업체 연동주소로서 실제 VOD 콘텐츠 주소와 다르다. 등록 레코드에는 실제 콘텐츠 URL, 실제 매니페스트 URL, 장기 접근 토큰 또는 DRM 키를 포함하지 않는다.

```json
{
  "providerPlaybackProfile": {
    "profileSchemaVersion": "instantplay.provider-playback-profile/1.0",
    ...
    "supportedDrmSystems": [
      {
        ...
        "licenseInformationSource": "grant-response"
      }
    ],
  ...
  "contentRegistration": {
    ...
    "playbackProfileIds": ["example-vod-profile-01"],
    "instantPlayPolicy": {
      "enabled": true,
...
###### 업체 재생 연동 프로필 및 콘텐츠 등록 레코드 필드 설명

| 필드 | 필수 여부 | 설명 |
|---|---|---|
| `profileSchemaVersion` | 필수 | 업체 재생 연동 프로필 형식의 버전 |
| `providerId` | 필수 | VOD 업체를 식별하고 권한 서비스와 연결하는 값 |
| `playbackProfileId` | 필수 | 개별 콘텐츠가 참조할 정적 재생 연동 프로필 식별자 |
| `grantService` | 필수 | 업체 권한 서비스 호출에 필요한 정적 연동정보 |
| `grantServiceId` | 필수 | 업체의 콘텐츠 재생권한 발급 서비스를 식별하는 값 |
...
| `supportedDrmSystems` | 선택 | 업체가 제공할 수 있는 DRM 체계와 라이선스 제공방식의 집합 |
| `licenseAcquisitionMode` | 선택 | 재생기가 라이선스 시스템에 직접 요청하는지 등을 나타내는 값 |
| `licenseInformationSource` | 선택 | 라이선스 주소·토큰을 권한 응답 등에서 취득함을 나타내는 값 |
| `supportedVideoCodecs` | 선택 | 업체가 제공할 수 있는 비디오 코덱의 집합 |
| `supportedAudioCodecs` | 선택 | 업체가 제공할 수 있는 오디오 코덱의 집합 |
| `profileVersion` | 필수 | 정적 재생 연동 프로필의 변경 버전 |
| `schemaVersion` | 필수 | 개별 콘텐츠 등록 레코드 형식의 버전 |
...
| `title` | 필수 | 추천 화면에 표시되는 콘텐츠 명칭 |
| `synopsis` | 선택 | 줄거리 또는 콘텐츠 설명 |
| `genres` | 선택 | 추천 분류에 사용하는 장르정보 |
| `cast` | 선택 | 검색·추천에 사용하는 출연진정보 |
| `contentRating` | 필수 | 노출·재생 가능 여부 판단에 사용하는 등급 |
| `posterImageUrl` | 선택 | 추천 화면용 대표 이미지의 위치이며 실제 VOD 영상주소와 다름 |
| `runningTimeSec` | 선택 | 재생시간 및 광고 요구시점 계산에 사용할 수 있는 값 |
| `availability` | 필수 | 제공지역과 제공기간 |
| `grantBinding` | 필수 | 업체 콘텐츠 식별자를 권한 발급 서비스와 연결하는 정보 |
| `playbackResourceId` | 필수 | 업체 권한 발급대상을 특정하는 재생 자산 식별자 |
| `playbackProfileIds` | 필수 | 해당 콘텐츠가 사용할 수 있는 정적 재생 연동 프로필 참조값의 집합 |
...
| `registrationVersion` | 필수 | 권한 요청과 추천 등록에 적용되는 등록정보 버전 |

업체 재생 연동 프로필은 호환성 판단에 필요한 정적 지원범위만 나타내며, 특정 사용자 세션에 사용할 실제 재생방식을 미리 확정하지 않는다. 등록 레코드에는 실제 VOD 영상 URL, 실제 매니페스트 URL, 장기 접근 토큰 및 DRM 키를 포함하지 않는 것이 바람직하다. 대표 이미지 URL은 추천용 메타데이터이며 VOD 콘텐츠의 실제 재생주소와 구별된다.
...
| `registrationVersion` | 필수 | 적용한 업체·콘텐츠 등록정보 버전 |
...
| `drmLicenseInformation` | DRM 적용 시 필수 | 단말 재생기가 기존 DRM 모듈을 이용하여 라이선스를 취득하는 데 필요한 정보 |
...
`drmLicenseInformation`은 DRM 콘텐츠 키 자체가 아니다. 즉시 콘텐츠 재생기(340)는 이 정보를 이용하여 업체 연계 DRM 라이선스 시스템에 직접 라이선스를 요청할 수 있다. 라이선스 응답과 콘텐츠 키의 복호화·사용은 기존 단말 DRM 모듈에서 수행되며, 플랫폼 서버 또는 즉시 재생 제어부가 콘텐츠 키를 평문으로 취득하거나 영구 저장하는 것을 요구하지 않는다.
...
플랫폼은 업체 단위의 정적 재생 연동 프로필을 저장하고, 업체 콘텐츠 식별자와 추천용 콘텐츠 메타데이터 및 검증된 프로필 참조값을 동일 플랫폼 콘텐츠 식별자에 연결한다. 시험 요청에서 업체가 반환한 방식이 등록 프로필과 검증 단말 능력의 공통범위인지 확인하고 실제 재생까지 성공한 프로필만 활성화한다. 실제 콘텐츠 URL은 저장하지 않으므로 주소 변경·만료·유출위험을 줄이고, 사용자 재생 시점에 업체의 최신 제공조건과 실제 단말 능력을 적용할 수 있다.
...
sequenceDiagram
    ...
    alt 광고 완료 확인
        ...
        CTRL->>PLAYER: 매니페스트·토큰·DRM 라이선스 정보와 재생명령
...
DRM이 적용된 실시예에서 즉시 콘텐츠 재생기는 업체가 반환한 DRM 라이선스 취득정보를 이용하여 업체 연계 라이선스 시스템과 직접 통신할 수 있다. 이 과정은 기존 DRM 규격을 사용할 수 있으며, DRM 알고리즘·보안 하드웨어 또는 콘텐츠 키 생성방식 자체는 본 발명의 필수 차별점이 아니다. 콘텐츠 키는 플랫폼 서버나 즉시 재생 제어부에 평문으로 전달되지 않고 기존 단말 DRM 모듈의 보호범위에서 처리된다.
...
| DRM 처리경계 | 플랫폼 서버는 DRM 라이선스 취득정보를 전달할 수 있으나 콘텐츠 키를 평문으로 취득·저장하지 않는다. 실제 라이선스 취득과 키 사용은 즉시 재생기와 기존 단말 DRM 모듈이 담당한다. |
...
VOD 업체 시스템이 제공하는 콘텐츠를 사용자 단말에서 즉시 재생하는 방법에 있어서,

즉시 재생 플랫폼이 상기 VOD 업체 시스템으로부터 상기 VOD 업체의 업체 식별자, 상기 VOD 업체가 부여한 업체 콘텐츠 식별자 및 상기 콘텐츠의 추천을 위한 콘텐츠 메타데이터를 등록받는 단계;

상기 즉시 재생 플랫폼이 상기 업체 콘텐츠 식별자와 상기 콘텐츠 메타데이터를 서로 연결하여 저장하되, 상기 콘텐츠의 실제 재생을 위한 네트워크 위치정보를 등록정보로 저장하지 않는 단계;

추천 시스템이 상기 콘텐츠 메타데이터에 기초하여 상기 콘텐츠를 사용자 단말에 제공하고, 상기 사용자 단말이 사용자로부터 상기 콘텐츠의 즉시 재생 선택을 수신하는 단계;

상기 즉시 재생 플랫폼이 상기 선택된 콘텐츠에 대응하는 필수 광고정보를 상기 사용자 단말에 제공하는 단계;

상기 사용자 단말이 상기 필수 광고정보에 대응하는 광고를 재생하고 상기 즉시 재생 플랫폼과 상기 광고의 시청 완료를 확인하는 단계;

상기 광고의 시청 완료가 확인된 경우에만 상기 즉시 재생 플랫폼이 상기 저장된 업체 콘텐츠 식별자를 이용하여 상기 VOD 업체 시스템에 상기 콘텐츠의 재생권한을 요청하는 단계;

상기 즉시 재생 플랫폼이 상기 VOD 업체 시스템으로부터 상기 콘텐츠의 한시적 재생권한 및 상기 콘텐츠의 실제 재생을 위한 재생정보를 취득하는 단계; 및

상기 사용자 단말의 즉시 콘텐츠 재생기가 상기 한시적 재생권한 및 상기 재생정보를 이용하여 상기 VOD 업체 시스템의 콘텐츠를 재생하는 단계를 포함하는 광고 조건부 즉시 콘텐츠 재생 방법.

### 청구항 2

청구항 1에 있어서, 상기 콘텐츠 메타데이터는 제목, 장르, 출연진, 이용등급, 줄거리, 대표 이미지, 재생시간, 제공지역 및 제공기간 중 하나 이상을 포함하고, 상기 네트워크 위치정보는 실제 콘텐츠 URL, 매니페스트 URL 및 장기 접근 토큰 중 하나 이상을 포함하는 방법.

### 청구항 3

청구항 1에 있어서, 상기 즉시 재생 플랫폼이 상기 등록된 업체 콘텐츠 식별자와 권한 요청정보를 이용하여 검증용 재생권한을 획득하고 상기 콘텐츠의 실제 재생 가능성을 시험하며, 상기 시험에 성공한 콘텐츠만 상기 추천 시스템에 등록하는 방법.
...
청구항 1에 있어서, 상기 즉시 재생 플랫폼은 상기 VOD 업체 시스템으로부터 상기 VOD 업체가 지원하는 스트리밍 방식, DRM 체계, 코덱 및 권한 요청규격 중 하나 이상을 포함하는 정적 재생 연동 프로필을 업체 단위로 등록받고, 개별 콘텐츠의 등록정보에는 실제 콘텐츠 URL 대신 상기 정적 재생 연동 프로필의 식별자를 연결하여 저장하는 방법.

### 청구항 7

청구항 6에 있어서, 상기 콘텐츠의 재생권한을 요청하는 단계는 상기 사용자 단말이 지원하는 스트리밍 방식, DRM 체계 및 코덱 중 하나 이상을 포함하는 단말 재생능력 정보를 상기 VOD 업체 시스템에 전송하는 단계를 포함하고, 상기 재생정보를 취득하는 단계는 상기 VOD 업체 시스템이 상기 정적 재생 연동 프로필과 상기 단말 재생능력 정보의 공통범위에서 선택한 스트리밍 방식, DRM 체계 또는 코덱을 나타내는 선택 재생방식을 취득하는 단계를 포함하는 방법.

### 청구항 8

청구항 1에 있어서, 상기 재생정보는 단기 콘텐츠 URL, 매니페스트 위치, 콘텐츠 접근 토큰, DRM 라이선스 주소, DRM 라이선스 요청 토큰 및 콘텐츠 보호방식 중 하나 이상을 포함하고 특정 콘텐츠·세션·단말·재생구간 또는 유효시간에 한정되며, DRM이 적용된 경우 상기 즉시 콘텐츠 재생기는 상기 DRM 라이선스 주소를 이용하여 업체 연계 DRM 라이선스 시스템으로부터 라이선스를 취득하고 콘텐츠 키는 상기 즉시 재생 플랫폼에 평문으로 전달되지 않는 방법.
...
VOD 업체 시스템에 업체·재생 연동 프로필 및 콘텐츠 등록방법을 제공하고, 상기 VOD 업체 시스템으로부터 업체 식별자, 업체가 지원하는 스트리밍 방식·DRM 체계·코덱·권한 요청규격 중 하나 이상을 포함하는 정적 재생 연동 프로필, 업체 콘텐츠 식별자, 추천용 콘텐츠 메타데이터 및 콘텐츠 재생권한 요청정보를 등록받는 VOD 업체·콘텐츠 관리부;

상기 정적 재생 연동 프로필을 업체 단위로 저장하고, 상기 업체 콘텐츠 식별자와 상기 추천용 콘텐츠 메타데이터 및 상기 정적 재생 연동 프로필의 식별자를 서로 연결하여 저장하되 실제 콘텐츠의 네트워크 위치정보를 등록정보로 저장하지 않는 즉시 재생 콘텐츠 저장소;

상기 콘텐츠 메타데이터를 이용하여 검증된 콘텐츠를 추천 시스템에 등록하는 추천 콘텐츠 등록부;

사용자 단말에서 선택된 콘텐츠에 대응하는 필수 광고정보를 제공하고 상기 사용자 단말과 광고 시청 완료를 확인하는 광고 처리부;

상기 광고 시청 완료가 확인된 경우에만 상기 저장소에서 상기 선택된 콘텐츠에 대응하는 업체 콘텐츠 식별자, 콘텐츠 재생권한 요청정보 및 정적 재생 연동 프로필을 조회하고, 상기 업체 콘텐츠 식별자와 사용자 단말의 재생능력 정보를 이용하여 상기 VOD 업체 시스템에 상기 콘텐츠의 한시적 재생권한을 요청하며, 상기 VOD 업체 시스템으로부터 상기 정적 재생 연동 프로필과 상기 사용자 단말의 재생능력 정보의 공통범위에서 선택된 재생방식, 상기 한시적 재생권한 및 실제 재생정보를 취득하는 콘텐츠 재생권한 획득부; 및
...
상기 실제 재생정보가 DRM 라이선스 취득정보를 포함하는 경우 상기 멀티미디어 재생기는 상기 DRM 라이선스 취득정보를 이용하여 업체 연계 DRM 라이선스 시스템으로부터 라이선스를 취득하고 기존 단말 DRM 모듈에서 콘텐츠 키를 처리하는 사용자 단말.
...
flowchart LR
    ...
    subgraph RUNTIME["사용자 재생 시점"]
        ...
        REQUEST --> RESPONSE["업체가 호환방식 선택<br/>단기 URL·토큰·라이선스 정보 발급"]
...
sequenceDiagram
    ...
    G-->>D: 선택 방식+매니페스트·토큰·라이선스 정보
    D->>V: 단기 정보로 매니페스트·암호화 콘텐츠 요청
    opt DRM 적용 콘텐츠
        D->>L: 단기 라이선스 토큰으로 라이선스 요청
        L-->>D: 단말 결합 DRM 라이선스
...
도 5는 플랫폼이 광고 완료 전에는 업체 권한을 요청하지 않고, 광고 완료 후 등록된 업체 콘텐츠 식별자와 단말 재생능력을 사용하여 업체가 선택한 실제 재생방식 및 재생정보를 취득하는 순서를 나타낸다. DRM 라이선스와 콘텐츠 키의 사용은 기존 단말 DRM 모듈에서 처리될 수 있다.
...
본 발명은 VOD 업체 시스템이 제공하는 콘텐츠를 업체 앱 설치 또는 업체 계정 로그인 없이 사용자 단말에서 즉시 재생하는 시스템 및 방법에 관한 것이다. 즉시 재생 플랫폼은 VOD 업체가 지원하는 스트리밍·DRM·코덱·권한 요청규격을 업체 단위의 정적 재생 연동 프로필로 저장하고, 개별 콘텐츠에 대하여 업체 콘텐츠 식별자, 프로필 참조값 및 추천용 콘텐츠 메타데이터를 연결하여 저장하되 실제 콘텐츠 URL 또는 장기 접근 토큰은 등록정보로 저장하지 않는다. 검증된 콘텐츠의 메타데이터는 추천 시스템에 등록된다. 사용자가 사용자 단말에서 콘텐츠를 선택하면 플랫폼은 선택 콘텐츠에 대응하는 필수 광고를 제공하고 사용자 단말과 광고 시청 완료를 확인한다. 광고 완료가 확인된 경우에만 플랫폼은 저장된 업체 콘텐츠 식별자와 단말 재생능력을 이용하여 해당 VOD 업체 시스템에 한시적 재생권한을 요청한다. VOD 업체 시스템은 등록 프로필과 단말 재생능력의 공통범위에서 실제 스트리밍·DRM·코덱 조합을 선택하고, 단기 URL, 접근 토큰, 매니페스트 위치 또는 DRM 라이선스 취득정보를 포함하는 실제 재생정보를 반환한다. 플랫폼은 취득한 선택 재생방식, 한시적 권한 및 실제 재생정보를 사용자 단말의 즉시 콘텐츠 재생기에 제공한다. DRM이 적용된 경우 즉시 콘텐츠 재생기는 업체 연계 DRM 라이선스 시스템으로부터 단말 결합 라이선스를 직접 취득하고 기존 단말 DRM 모듈에서 콘텐츠 키를 처리한다. 이에 따라 플랫폼은 실제 VOD 콘텐츠와 장기 재생주소 또는 DRM 키를 보유하지 않고도 복수 업체 콘텐츠를 추천·재생할 수 있고, 광고 완료와 업체 권한 발급을 연결하며, VOD 업체의 기존 권한·DRM 체계를 유지할 수 있다.

### 5.2 출원 전략

가장 넓은 독립항은 다음 결합을 중심으로 한다.

> `업체 제공 콘텐츠 식별자와 추천용 메타데이터의 등록` + `실제 재생정보의 등록단계 미저장` + `콘텐츠 대응 광고 완료 확인` + `업체 식별자 기반 권한 요청` + `업체 응답으로 실제 재생정보 지연 취득` + `플랫폼 즉시 재생기에서 콘텐츠 재생`

1. 방법 독립항은 등록 데이터 분리부터 광고·권한·재생까지의 시간적 순서를 보호한다.
2. 시스템 독립항은 플랫폼 저장소, 광고 처리부, 권한 획득부 및 단말 재생기의 상호관계를 보호한다.
3. 사용자 단말 독립항은 사용자 인터페이스, 즉시 재생 제어부, 광고 시청 확인부 및 재생기의 결합을 보호한다.
4. 등록 검증과 검증 성공 콘텐츠의 추천 등록은 종속항과 별도 플랫폼 독립항으로 준비한다.
5. 업체 단위 정적 재생 연동 프로필, 콘텐츠별 프로필 참조, 단말 능력 전달 및 업체의 호환방식 선택은 종속항과 구체화된 시스템 독립항으로 보호한다.
6. DRM 라이선스의 단말 직접 취득과 콘텐츠 키의 플랫폼 비노출은 기존 DRM 자체를 청구하기보다 플랫폼·업체·단말의 처리경계를 한정하는 종속항으로 보호한다.
...
- 업체 단위로 등록·저장: 정적 재생 연동 프로필, 지원 스트리밍·DRM·코덱 범위, 권한 요청규격 및 라이선스 제공방식
- 콘텐츠 단위로 등록·저장: 업체 식별자, 업체 콘텐츠 식별자, 플랫폼 콘텐츠 식별자, 추천용 콘텐츠 메타데이터, 재생 연동 프로필 참조값, 권한 서비스 매핑, 광고정책, 검증상태
- 등록 시 미저장: 실제 VOD 콘텐츠 URL, 실제 매니페스트 URL, 장기 접근 토큰, DRM 키
- 광고 완료 후 요청에 사용: 사용자 단말의 스트리밍·DRM·코덱 재생능력 정보
- 광고 완료 후 취득: 업체가 선택한 실제 재생방식, 한시적 재생권한, 단기 URL, 접근 토큰, 매니페스트 위치, DRM 라이선스 취득정보
- 단말 내부에서 처리: 업체 연계 DRM 라이선스 시스템이 발급한 라이선스와 콘텐츠 키
...
9. DRM 라이선스 취득정보만 단말에 전달하고 콘텐츠 키는 플랫폼 서버가 취득하지 않는 처리경계

제8항과 제9항의 구조는 기존 스트리밍·DRM 알고리즘 자체를 신규한 것으로 주장하기 위한 것이 아니라, 실제 재생정보를 등록 시 저장하지 않는 본 발명이 다양한 단말에서 실시될 수 있는 구체적 정보처리 관계와 시스템 경계를 보강하기 위한 것이다.
...
9. 실제 재생정보 중 DRM 라이선스 취득정보는 단말에 제공하되 콘텐츠 키는 플랫폼 서버에 전달하지 않는 구조
...
- DRM 라이선스 취득정보의 전달과 플랫폼 서버·단말 DRM 모듈의 처리경계
- 단기 URL·토큰의 유효범위와 재사용 제한
- 즉시 재생기의 상태전이와 광고 미완료 시 차단동작
- 업체 등록 검증과 추천 활성화 기준
- 적어도 하나의 정상 흐름과 실패 흐름

본 직무발명서에는 위 수준의 실시예와 JSON·상태·순서도를 포함하였다. 정적 재생 연동 프로필, 단말 재생능력, 업체 선택 재생방식 및 단말 DRM 라이선스 흐름도 구체화하였다. 따라서 아이디어 수준을 넘어 명세서 초안을 작성할 수 있는 기반은 마련되어 있다. 다만 출원 전에 광고 완료 확인정보의 실제 생성주체, 업체 권한 API의 최소 필드, 지원할 재생 연동 프로필의 범위, 실제 재생정보와 DRM 라이선스 토큰의 수명 및 단말 전달방식을 내부 설계로 한 번 더 확정하면 실시가능요건과 청구항 명확성을 강화할 수 있다.
...
| 데이터 구조 | 업체 재생 연동 프로필, 콘텐츠 등록 레코드, 단말 능력, 권한 요청·응답 및 필드별 설명 기재 | 완료 |
...
| 실제 구현값 | 제품명, API 명칭, 광고 확인방식, 지원 프로필, 단말 능력 필드, URL·라이선스 토큰 수명 | 출원 전 일부 확정 필요 |
