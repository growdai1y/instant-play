...
> **문서 안내**: 제1장은 발명을 처음 접하는 심사자를 위한 개요, 제2장은 구현 가능한 수준의 구체적 설명, 제3장은 변리사에게 전달할 권리화 아이디어, 제4장은 도면과 전체 데이터 형식 예시이다. 본문의 JSONC는 핵심 관계만 보여 주는 축약 예시이며, 전체 형식은 제4장을 참조한다.
>
...
flowchart LR
    ...
    D --> E{"사전 시험 합격 구성·버전과 일치?"}
    E -->|일치| F["플랫폼 재생기에서 즉시 재생"]
    E -->|불일치| G["재생정보 사용 차단·재검증"]
```
...
이 장은 완성된 특허 청구항이 아니라, 회사가 발명의 가치와 출원 방향을 검토하고 변리사가 청구항을 작성할 때 사용할 기술 관계를 정리한 것이다. 구체적인 청구항 문언과 범위는 정식 선행기술 조사와 실제 연동 구조 확인 후 결정한다.
...
| 주요 확인 문헌 | 가까운 내용 | 본 발명에서 더 구체화한 관계 |
|---|---|---|
| [US7711647B2, *Digital rights management in a distributed network*](https://patents.google.com/patent/US7711647B2/en) | 사업 규칙 완료 후 외부 DRM 시스템이 데이터를 확인하고 라이선스를 발급 | 콘텐츠별 조건·결과 규격, 업체 처리상태, 실제 시험 합격 구성과 실행 응답의 버전 대조를 하나의 연쇄로 연결 |
| [US11805132B2, *Location specific temporary authentication system*](https://patents.google.com/patent/US11805132B2/en) | 조건 충족 후 외부 콘텐츠 제공자의 승인을 받아 임시 접근 제공 | 플랫폼의 조건 결과 형식과 업체 확인 규칙, 콘텐츠별 검증 재생구성집합을 함께 사용 |
| [US12111891B2](https://patents.google.com/patent/US12111891B2/en) / [EP3491562B1](https://patents.google.com/patent/EP3491562B1/en) | 단말 지원 범위에 맞는 재생 방식 선택과 후속 요청 비교 | 업체 권한 발급과 별도로, 실제 콘텐츠 시험 합격 구성·버전이 실행 허용과 추천 상태를 지배 |

위 표는 직무발명 검토를 위한 예비 정리이다. 정식 출원 전에는 발명 기준일을 기준으로 선행기술, 패밀리, 법적 상태 및 결합 가능성을 변리사가 다시 조사해야 한다.

### 라. 변리사 검토 시 기술적으로 확인할 사항
...
도 2는 조건 처리 후 콘텐츠 접근을 허용하는 일반적인 구조를 나타낸다. 본 발명은 이러한 구조 자체가 아니라, 조건별 결과 규격, 업체의 수락·처리상태·권한 발급, 등록 시 검증된 재생구성과 실행 재생구성 간 일치 제어를 결합한 점에 특징이 있다.
...
> 이 항은 기술팀과 변리사가 필요할 때 확인하는 참고 부록이다. 이하 JSONC는 필수 API 규격을 확정한 것이 아니라, 기준 실시예에서 데이터가 어떤 식별자와 버전으로 연결되는지를 설명하는 전체 예시이다. `//` 주석은 문서 설명용이므로 실제 전송 시 제거한다. 발명의 공통 기술관계는 제1장과 제2장의 설명을 기준으로 하며, 필드명·중첩 구조·전송 방식·인증 방식은 실제 업체 연동 환경에 따라 달라질 수 있다.
